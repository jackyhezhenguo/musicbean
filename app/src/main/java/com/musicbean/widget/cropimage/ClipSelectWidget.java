...
public class ClipSelectWidget extends RelativeLayout implements
        OnTouchListener {
    private static final float MAX_SCALE = 10.0f;
    private int DEFAULT_SIZE = 640;

    private ClipShadowView mClipView;
    ...
    public ClipSelectWidget(Context context) {
        super(context);
        init(context);
    }

    public ClipSelectWidget(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public void setScaleSizeMax(int size){
        DEFAULT_SIZE = size;
    }

    public ClipSelectWidget(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        ...
        LayoutInflater.from(getContext()).inflate(R.layout.clip_picture_layout,
                this);
        mClipView = (ClipShadowView) findViewById(R.id.clipview);
        mSouceImageView = (ImageView) findViewById(R.id.src_pic);
        mSouceImageView.setOnTouchListener(this);
        mClipViewWidth = dm.widthPixels * 1 / 2;
        mClipViewHeight = dm.widthPixels * 1 / 2;
...
//		}
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        if (isChange) {
            isChange = false;
            scaleFit(mBitmap, mClipViewWidth * 4 / 3, mClipViewHeight * 4 / 3);
            mSouceImageView.setImageMatrix(matrix);
            centerClip(mBitmap);
            mSouceImageView.setImageMatrix(matrix);
        }

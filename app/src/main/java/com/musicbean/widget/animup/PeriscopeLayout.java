...
package com.musicbean.widget.animup;
...
public class PeriscopeLayout extends RelativeLayout {
    ...
    private int[] mDrawableIds = {R.drawable.yinfu1, R.drawable.yinfu2, R.drawable.yinfu3, R.drawable.yinfu4, R.drawable.yinfu5};

    private void init() {

        //初始化显示的图片
        drawables = new Drawable[mDrawableIds.length];
        for (int i = 0; i < mDrawableIds.length; i++) {
            drawables[i] = getResources().getDrawable(mDrawableIds[i]);
        }

        Drawable yinfu = drawables[0];
        //获取图的宽高 用于后面的计算
        //注意 我这里3张图片的大小都是一样的,所以我只取了一个
        dHeight = yinfu.getIntrinsicHeight();
        dWidth = yinfu.getIntrinsicWidth();
